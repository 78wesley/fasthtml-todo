...
def get(auth):
    ...
    add = Form(Group(new_inp, Button("Add")), hx_post="/", target_id="todo-list", hx_swap="afterbegin")
...
async def get(id: int):
    ...
    res = Form(Group(Input(id="title"), Button("Save")), Hidden(id="id"), Checkbox(id="done", label="Done"), Textarea(id="details", name="details", rows=10), hx_put="/", target_id=f"todo-{id}", id="edit")
...
async def get(id: int):
    ...
    btn = Button("delete", hx_delete=f"/todos/{todo.id}", target_id=f"todo-{todo.id}", hx_swap="outerHTML")
